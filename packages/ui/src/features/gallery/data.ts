import { GalleryCollection } from '../../types/media';

export const galleryCollections: GalleryCollection[] = [
  {
    id: 'exteriores',
    label: 'EXTERIORES',
    images: [
      { src: '/images/exterior/IMG_1215.jpeg', alt: 'Entorno natural de Coliumo' },
      { src: '/images/exterior/IMG_1216.jpeg', alt: 'Área exterior rodeada de naturaleza' },
      { src: '/images/exterior/IMG_1217.jpeg', alt: 'Fachada de las cabañas' },
      { src: '/images/exterior/IMG_1218.jpeg', alt: 'Paisaje costero de Coliumo' },
      { src: '/images/exterior/IMG_1219.jpeg', alt: 'Vista panorámica del entorno' },
      { src: '/images/exterior/IMG_1220.jpeg', alt: 'Espacios exteriores de las cabañas' },
      { src: '/images/exterior/IMG_1228.jpeg', alt: 'Ambiente natural de la zona' },
      { src: '/images/exterior/IMG_1229.jpeg', alt: 'Vista exterior al atardecer' },
      { src: '/images/exterior/IMG_1230.jpeg', alt: 'Entorno de las cabañas' },
      { src: '/images/exterior/IMG_1234.jpeg', alt: 'Área común exterior' },
      { src: '/images/exterior/IMG_1235.jpeg', alt: 'Entorno natural de las cabañas' },
      { src: '/images/exterior/IMG_1236.jpeg', alt: 'Paisaje de Tres Morros' },
      { src: '/images/exterior/IMG_1238.jpeg', alt: 'Naturaleza de Coliumo' },
      { src: '/images/exterior/IMG_1239.jpeg', alt: 'Vista general del complejo' },
    ],
  },
  {
    id: 'interiores',
    label: 'INTERIORES',
    images: [
      { src: '/images/galeria/interior/1.jpg', alt: 'Sala de estar con luz natural' },
      { src: '/images/galeria/interior/2.jpg', alt: 'Comedor familiar acogedor' },
      { src: '/images/galeria/interior/3.jpg', alt: 'Dormitorio principal con detalles en madera' },
      { src: '/images/galeria/interior/4.jpg', alt: 'Ambiente interior iluminado con calidez' },
    ],
  },
  {
    id: 'puntos-turisticos',
    label: 'PUNTOS TURÍSTICOS',
    images: [
      { src: '/images/galeria/puntos-turisticos/IMG_1471.jpeg', alt: 'Mirador natural de Coliumo' },
      { src: '/images/galeria/puntos-turisticos/IMG_1479.jpeg', alt: 'Senderos costeros para caminar' },
      { src: '/images/galeria/puntos-turisticos/IMG_1519.jpeg', alt: 'Vista panorámica del océano Pacífico' },
      { src: '/images/galeria/puntos-turisticos/IMG_1720.png', alt: 'Atardecer desde los miradores' },
    ],
  },
  {
    id: 'playas',
    label: 'PLAYAS',
    images: [
      { src: '/images/galeria/playas/dji_fly_20251201_183018_39_1764693967315_photo_optimized.jpeg', alt: 'Vista aérea de playas de Coliumo' },
      { src: '/images/galeria/playas/IMG_1133.jpeg', alt: 'Playa amplia con arena dorada' },
      { src: '/images/galeria/playas/IMG_1717.jpeg', alt: 'Acantilados y mar turquesa' },
      { src: '/images/galeria/playas/IMG_1718.jpeg', alt: 'Olas rompiendo al atardecer' },
      { src: '/images/galeria/playas/IMG_1719.jpeg', alt: 'Sendero costero junto a la playa' },
    ],
  },
];

